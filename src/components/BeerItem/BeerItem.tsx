...
const BeerItem: React.FC<Beer> = (props) => {
  ...
  return (
    <li className='item-wrapper' onClick={handleClick}>
      ...
      <button className='item-button item-button__fav' onClick={() => handleFav && handleFav(props)}>
        <svg className='fav-icon' xmlns="http://www.w3.org/2000/svg" viewBox="0 0 260 245">
          <path d="m56,237 74-228 74,228L10,96h240" fill={favorites.includes(props.id) ? '#00b4e6' : '#829094'}/>
        </svg>
      </button>

      {expanded && (
        <div className={`item-details ${!expanded && 'item-details__hidden'}`}>
          ...
          <div className='details-food'>
            ...
            <ul>{props.food_pairing.map((food: string) => (<li>{food}</li>))}</ul>
          </div>
        </div>
        )
      }
